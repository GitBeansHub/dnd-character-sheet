...
nav_text_size = 0.09 # Nav Text Size (as fraction of width)
...
class Node:
    ...
    def name_ui(self) -> str:
        return self.name_disk[1:]  # hides the prefix character
...
def draw_nav(left_w, nav_w, top_h, w, h):
    ...
    for i, child in enumerate(tree.children):
        ...
        imgui.same_line()
        if imgui.button(f"{child.name_ui}##{child.name_disk}"):
